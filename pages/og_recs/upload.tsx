...
interface UploadRecordingsForm {
	title: string;
	description: string;
}

const Upload: NextPage = () => {
	const { register, handleSubmit } = useForm<UploadRecordingsForm>();
	const [uploadProduct, { loading, data }] = useMutation("/api/recordings");
	const onValid = (data: UploadRecordingsForm) => {
		if (loading) return;
		uploadProduct(data);
	};
	return (
